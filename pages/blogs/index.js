        ...
        <div>
            {blogs.map(blog => (
                <Link href={`/blogs/${ blog.id }`} key={ blog.id } >
                    <div className="px-6 py-5 shadow-md my-2 rounded-sm border-s-8 border-s-white hover:border-s-blue-500">
                        <div className="md:flex">
                            <Image className="rounded-md md:mx-3 my-2 mx-0 md:h-28 md:w-40 w-full" src={ blog.image } height={800} width={800}/>
                            <div className="">
                                <h3 className="text-xl font-medium text-zinc-700 ">{ blog.title }</h3>
